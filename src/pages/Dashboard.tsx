...
import { TrendingUp, Users, Target, Gem, MapPin, ArrowUpRight, Sparkles } from "lucide-react";
import { useNavigate } from "react-router-dom";
import { ScoreBar } from "@/components/ScoreIndicators";

const highPriority = mockRetailers.filter(r => r.priorityScore >= 85).sort((a, b) => b.priorityScore - a.priorityScore);
const pipelineValue = getTotalPipelineValue();

const stats = [
  { label: "Total Prospects", value: mockRetailers.length.toString(), icon: Users, sub: "Across South West UK", accent: false },
  { label: "High Priority", value: highPriority.length.toString(), icon: Target, sub: "Ready to pursue", accent: false },
  { label: "Pipeline Value", value: `£${(pipelineValue / 1000).toFixed(0)}k`, icon: TrendingUp, sub: "Est. wholesale potential", accent: true },
  { label: "Active Outreach", value: mockRetailers.filter(r => ['contacted', 'meeting_needed', 'contact_planned'].includes(r.pipelineStage)).length.toString(), icon: Sparkles, sub: "Prospects in progress", accent: false },
];
  ...
  return (
    <div className="page-container">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        {stats.map((s) => (
          <div key={s.label} className={`stat-card group cursor-default ${s.accent ? 'border-gold/30 bg-champagne/20' : ''}`}>
            <div className="flex items-start justify-between mb-3">
              <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${s.accent ? 'gold-gradient shadow-sm' : 'bg-muted'}`}>
                <s.icon className={`w-4.5 h-4.5 ${s.accent ? 'text-card' : 'text-muted-foreground'}`} strokeWidth={1.5} />
              </div>
            </div>
      ...
      <div className="card-premium p-6">
        <div className="flex items-center justify-between mb-5">
          <h3 className="text-lg font-display font-semibold text-foreground">Priority Prospects</h3>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-border/30">
                {['Retailer', 'Location', 'Category', 'Fit Score', 'Priority', 'Est. Spend'].map(h => (
              ...
              {highPriority.slice(0, 6).map((r) => (
                <tr key={r.id} onClick={() => navigate(`/retailer/${r.id}`)} className="border-b border-border/15 hover:bg-champagne/20 transition-colors cursor-pointer group">
                  <td className="py-3.5 pl-0">
                    <span className="text-sm font-medium text-foreground group-hover:text-gold-dark transition-colors">{r.name}</span>
                  </td>
                  ...
                  <td className="py-3.5">
                    <div className="w-20"><ScoreBar score={r.fitScore} label="" /></div>
                  </td>
                  <td className="py-3.5">
