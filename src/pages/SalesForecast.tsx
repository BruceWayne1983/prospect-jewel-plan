import { mockRetailers, salesForecast, promotionalCampaigns, performanceBenchmarks, COUNTIES } from "@/data/mockData";
import { TrendingUp, CalendarDays, Target, Zap, Gift, BarChart3 } from "lucide-react";
...
const countyForecast = COUNTIES.map(c => {
  ...
  const value = rs.reduce((s, r) => {
    const m = r.estimatedSpendBand.match(/£([\d,]+)/);
    return s + (m ? parseInt(m[1].replace(',', '')) : 0);
  }, 0);
  const qualified = rs.filter(r => r.qualificationStatus === 'qualified').length;
  return { county: c, prospects: rs.length, qualified, annualEstimate: value };
}).filter(Boolean) as { county: string; prospects: number; qualified: number; annualEstimate: number }[];
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div key={c.id} className={`rounded-xl p-4 border ${c.status === 'active' ? 'border-gold/20 bg-champagne/10' : c.status === 'upcoming' ? 'border-border/20 bg-cream/30' : 'border-border/10'}`}>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className={`text-[9px] px-2 py-0.5 rounded-full font-medium ${
                    c.status === 'active' ? 'bg-success-light text-success' :
                    c.status === 'upcoming' ? 'bg-info-light text-info' :
                ...
                <div className="flex gap-1.5 mt-2">
                  {c.topRetailers.slice(0, 3).map(r => (
                    <span key={r} className="text-[9px] px-2 py-0.5 rounded-full bg-muted text-muted-foreground">{r}</span>
                  ))}
                </div>
              </div>
            ))}
        ...
        <div className="card-premium p-6">
          <div className="flex items-center gap-2.5 mb-5">
            ...
            <h3 className="text-lg font-display font-semibold text-foreground">Forecast by County</h3>
          </div>
          <div className="space-y-3">
            {countyForecast.sort((a, b) => b.annualEstimate - a.annualEstimate).map(c => (
              <div key={c.county} className="flex items-center gap-4">
                <span className="text-sm text-foreground w-28">{c.county}</span>
                <div className="flex-1 h-2 bg-cream rounded-full overflow-hidden">
                  <div className="h-full bg-gold/50 rounded-full transition-all duration-700" style={{ width: `${(c.annualEstimate / countyForecast[0].annualEstimate) * 100}%` }} />
                </div>
                <div className="flex gap-4 text-xs min-w-[140px] justify-end">
                  <span className="text-muted-foreground">{c.qualified} qualified</span>
                  <span className="text-gold-dark font-semibold">£{(c.annualEstimate / 1000).toFixed(0)}k</span>
                </div>
              </div>
            ))}
          </div>
